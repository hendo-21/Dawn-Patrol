...
def send_notification(surfer_count, api_key, endpoint_url):
    ...
    if surfer_count is None:
        requests.post(
            endpoint_url,
            json={"text": "Could not get surfer count this morning. Check stream for issues."},
    ...
    if 0 <= surfer_count <= 20:
        ...
        msg = f"⚠️ You'd be out there with {surfer_count} of your closest friends, "
        "but you could still catch a few."
    else:
        msg = (
            f"❌ Find something else to do today. It's packed: {surfer_count} surfers in the water."
        )
